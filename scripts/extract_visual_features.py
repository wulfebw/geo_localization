'''
author: ahmed osman
email : ahmed.osman99 AT GMAIL
'''

"""
todo:
(1) mean image values for my dataset
(2) testing to make sure that this the features are in order

notes:
primary difficulties
(1) understanding image formating
	- preprocessing
	- size
	- subtracting mean
	- cropping
	- quality

    [conclusion]    - it seems like I've managed to avoid all this
                    - but that will be made definite eventually

(2) understanding col vs row representations
	- are rows features or examples? seems like features

    [conclusion]    - avoided as well it seems
                    - reading in data I'll need to figure this out, but might just copy whatever he has

(3) format of features 
	- seem likely to be the same data type as np.zeros, which is <type 'numpy.float64'>

otherwise seems quite straight forward to be honest


 x = '/Users/wulfe/Dropbox/Start/scripts/machine_learning/animals.jpg'
>>> y = '/Users/wulfe/Dropbox/Start/smile/data/frames/0209_f01_m01_5082_gray.jpg'
>>> import caffe
>>> caffe.io.load_image(x)



scp -i ~/Documents/Other/keys/EC2_Key.pem ~/Dropbox/Start/scripts/machine_learning/caffe_feature_extraction.py ubuntu@54.212.53.49:/home/ubuntu/scripts/caffe_feature_extraction.py

scp -i ~/Documents/Other/keys/EC2_Key.pem /Users/wulfe/Dropbox/Start/smile/data/_temp/something.prototxt ubuntu@54.212.53.49:/home/ubuntu/models/proto/default.prototxt

wget http://www.robots.ox.ac.uk/~vgg/software/very_deep/caffe/VGG_ILSVRC_16_layers.caffemodel   // from ubuntu

wget https://s3-us-west-2.amazonaws.com/bsdsframes/0209_f01_m01.zip     // from ubuntu
    
    followed by moving the frames out of their hierarchy and into the frames dir

need to test this

todo:
(1) separate out the extract features and main into logical, easily callable functions
(2) need unified way to call this shit

"""


import os
import sys
import csv
import time
import glob
import caffe
import argparse
import scipy.io

import numpy as np


# don't know what this is
def reduce_along_dim(img , dim , weights , indicies): 
    '''
    Perform bilinear interpolation given along the image dimension dim
    -weights are the kernel weights 
    -indicies are the crossponding indicies location
    return img resize along dimension dim
    '''
    other_dim = abs(dim-1)       
    if other_dim == 0:  #resizing image width
        weights  = np.tile(weights[np.newaxis,:,:,np.newaxis],(img.shape[other_dim],1,1,3))
        out_img = img[:,indicies,:]*weights
        out_img = np.sum(out_img,axis=2)
    else:   # resize image height     
        weights  = np.tile(weights[:,:,np.newaxis,np.newaxis],(1,1,img.shape[other_dim],3))
        out_img = img[indicies,:,:]*weights
        out_img = np.sum(out_img,axis=1)
        
    return out_img

# this is a more in-depth thing than I initially thought
# prob read that paper
def cubic_spline(x):
    '''
    Compute the kernel weights 
    See Keys, "Cubic Convolution Interpolation for Digital Image
    Processing," IEEE Transactions on Acoustics, Speech, and Signal
    Processing, Vol. ASSP-29, No. 6, December 1981, p. 1155.
    '''
    absx   = np.abs(x)
    absx2  = absx**2
    absx3  = absx**3 
    kernel_weight = (1.5*absx3 - 2.5*absx2 + 1) * (absx<=1) + (-0.5*absx3 + 2.5* absx2 - 4*absx + 2) * ((1<absx) & (absx<=2))
    return kernel_weight
 
# same as cub_spline
def contribution(in_dim_len , out_dim_len , scale ):
    '''
    Compute the weights and indicies of the pixels involved in the cubic interpolation along each dimension.
    
    output:
    weights a list of size 2 (one set of weights for each dimension). Each item is of size OUT_DIM_LEN*Kernel_Width
    indicies a list of size 2(one set of pixel indicies for each dimension) Each item is of size OUT_DIM_LEN*kernel_width
    
    note that if the entire column weights is zero, it gets deleted since those pixels don't contribute to anything
    '''
    kernel_width = 4
    if scale < 1:
        kernel_width =  4 / scale
        
    x_out = np.array(range(1,out_dim_len+1))  
    #project to the input space dimension
    u = x_out/scale + 0.5*(1-1/scale)
    
    #position of the left most pixel in each calculation
    l = np.floor( u - kernel_width/2)
  
    #maxium number of pixels in each computation
    p = int(np.ceil(kernel_width) + 2)
    
    indicies = np.zeros((l.shape[0],p) , dtype = int)
    indicies[:,0] = l
      
    for i in range(1,p):
        indicies[:,i] = indicies[:,i-1]+1
    
    #compute the weights of the vectors
    u = u.reshape((u.shape[0],1))
    u = np.repeat(u,p,axis=1)
    
    if scale < 1:
        weights = scale*cubic_spline(scale*(indicies-u ))
    else:
        weights = cubic_spline((indicies-u))
         
    weights_sums = np.sum(weights,1)
    weights = weights/ weights_sums[:, np.newaxis] 
    
    indicies = indicies - 1    
    indicies[indicies<0] = 0                     
    indicies[indicies>in_dim_len-1] = in_dim_len-1 #clamping the indicies at the ends
    
    valid_cols = np.all( weights==0 , axis = 0 ) == False #find columns that are not all zeros
    
    indicies  = indicies[:,valid_cols]           
    weights    = weights[:,valid_cols]
    
    return weights , indicies
     
# resize an image
def imresize(img , cropped_width , cropped_height):
    '''
    Function implementing matlab's imresize functionality default behaviour
    Cubic spline interpolation with antialiasing correction when scaling down the image.
    
    '''
    
    
    width_scale  = float(cropped_width)  / img.shape[1]
    height_scale = float(cropped_height) / img.shape[0] 
    
    if len(img.shape) == 2: #Gray Scale Case
        img = np.tile(img[:,:,np.newaxis] , (1,1,3)) #Broadcast 
    
    order   = np.argsort([height_scale , width_scale])
    scale   = [height_scale , width_scale]
    out_dim = [cropped_height , cropped_width] 
    
    
    weights  = [0,0]
    indicies = [0,0]
    
    for i in range(0 , 2):
        weights[i] , indicies[i] = contribution(img.shape[ i ],out_dim[i], scale[i])
    
    for i in range(0 , len(order)):
        img = reduce_along_dim(img , order[i] , weights[order[i]] , indicies[order[i]])
        
    return img

def crop_image(img, cropped_width, cropped_height):
	"""
	:description: traditional image crop. The reason for doing this is that it allows the individual to consititute a large fraction of the analyzed pixels, since the img must ultimately be scaled to 224*224.
	"""
	width = img.shape[1]
	height = img.shape[0]

	if cropped_width > width or cropped_width < 0:
		cropped_width = width
	if cropped_height > height or cropped_height < 0:
		cropped_height = height

	x = (width - cropped_width) / 2
	y = (height - cropped_height) / 2
	return img[y:y+cropped_height, x:x+cropped_width, :]

# all the preprocessing of an image necessary
def preprocess_image(img):
    '''
    Preprocess an input image before processing by the caffe module.
    
    
    Preprocessing include:
    -----------------------
    1- Converting image to single precision data type
    2- Resizing the input image to cropped_dimensions used in extract_features() matlab script
    3- Reorder color Channel, RGB->BGR
    4- Convert color scale from 0-1 to 0-255 range (actually because image type is a float the 
        actual range could be negative or >255 during the cubic spline interpolation for image resize.
    5- Subtract the VGG dataset mean.
    6- Reorder the image to standard caffe input dimension order ( 3xHxW) 
    '''
    img = img.astype(np.float32)							# what are they before?
    img = crop_image(img, 300, 240)
    img = imresize(img,224,224) #cropping the image 		# should probably do this
    img = img[:,:,[2,1,0]] #RGB-BGR 						# not sure what this is
    img = img*255											# scale the pixel weights
    
    # need to set this with my mean
    #mean = np.array([103.939, 116.779, 123.68]) #mean of the vgg # vgg = the team that did something, I would guess that this is the mean pixel values accross the dataset
   	# this may be screwing things up
    mean = np.array([133.773, 135.8895, 137.037])

    for i in range(0,3):
        img[:,:,i] = img[:,:,i] - mean[i] #subtracting the mean		# supported here
    img = np.transpose(img, [2,0,1])	# transpose to get standard order, might read up on this
    return img #HxWx3		# this is possibly a good habit - labeling return values

def extract_frame_number_from_filepath(img_path):
    try:
    	img_path = os.path.splitext(img_path)[0]
        return int(img_path.split('_')[-1])
    except ValueError as e:
        raise ValueError('invalid path of: {}'.format(img_path))

def sort_img_paths(img_paths):
	return sorted(img_paths, key=extract_frame_number_from_filepath)

# main method
def extract_visual_features(input_dir, caffe_net):
    '''
    # uses VGG_ILSVRC_16_layers.caffemodel, where can I get that I wonder?
    # extracts 4096 features looks like
    Function using the caffe python wrapper to extract 4096 from VGG_ILSVRC_16_layers.caffemodel model
    
    Inputs:
    ------
   
    
    Output:
    -------
    features           : return the features extracted  				# what is their form?
    '''
    print('extract_visual_features called')
    files = glob.glob('{0}/*.jpg'.format(input_dir))
    # print files
    # sort b/c need the features to be in sequence
    imgs = sort_img_paths(files)
    image_paths = imgs #[os.path.join(input_directory, f) for f in imgs]

    # model_def_path = '/Users/wulfe/Dropbox/Start/smile/data/_temp/something.prototxt'
    # # model_def_path = '/home/ubuntu/models/proto/default.prototxt'
    # model_path = '/Users/wulfe/Dropbox/Start/scripts/distributed_file_processing/tests/test_data/models/VGG_ILSVRC_16_layers.caffemodel'
    # # model_path = '/home/ubuntu/models/caffe/VGG_ILSVRC_16_layers.caffemodel'
    # WITH_GPU = False
    batch_size = 10


    # # flag to set GPU on/off, might have been getting an error with this
    # if WITH_GPU:
    #     caffe.set_mode_gpu()
    # else:
    #     caffe.set_mode_cpu()
    
    # # the classifier
    # # prototxt file
    # # .caffemodel file
    # # img dimensions
    # # raw scale of pixels (max)
    # # channel_swap ?
    # # the mean image, why giving this here is in preprocessing it was subtracted?
    # caffe_net = caffe.Classifier(model_def_path, model_path, image_dims = (224,224), raw_scale = 255,channel_swap=(2,1,0), mean = np.array([133.773, 135.8895, 137.037]))
    # feature array, the rows are the features and the columns are the different images
    feats = np.zeros((4096 , len(image_paths)))
    


    # from 0 to the number of images, in chuncks of batch_size
    for b in range(0 , len(image_paths) , batch_size):
        start = time.time()
    	# crate a new list of images
        list_imgs = []
        # for each img in each batch (use batches because memory constraints I assume)
        for i in range(b , b + batch_size ):
        	# if still within index, then load the image
            if i < len(image_paths):
                try:
                    list_imgs.append( np.array( caffe.io.load_image(image_paths[i]) ) ) #loading images HxWx3 (RGB)
                except Exception as e:
                    print image_paths[i]
                    raise e
            # if we've run out of images, then some fake values to make the total num of imgs divisible by convenient number (they are ignored)
            else:
                list_imgs.append(list_imgs[-1]) #Appending the last image in order to have a batch of size 10. The extra predictions are removed later..
           
        
        # the input to caffe?
        # list of images, each preprocessed, how does this not run out of memory?
        caffe_input = np.asarray([preprocess_image(in_) for in_ in list_imgs]) #preprocess the images
        # print('len caffe input: {}'.format(caffe_input))

        # call the net forward on _all of the data_
        predictions = caffe_net.forward(data = caffe_input)
        # getting the first item in the output (so these are probably the features)
        # then transposing it
        predictions = predictions[caffe_net.outputs[0]].transpose()
        # only add real predictions, not the fake ones
        if i < len(image_paths):
            feats[:,b:i+1] = predictions
            n = i+1
        else:
            n = min(batch_size , len(image_paths) - b) 
            feats[:,b:b+n] = predictions[:,0:n] #Removing extra predictions, due to the extra last image appending.
            n += b 
        print "%d out of %d done....."%(n ,len(image_paths))
        end = time.time()  
        print('batch time: {}'.format(end - start))

    return feats
     


    
if __name__ == '__main__':
    # using arg parser, lets adopt this
    parser = argparse.ArgumentParser()
    parser.add_argument('--model_def_path',dest='model_def_path', type=str , help='Path to the VGG_ILSVRC_16_layers model definition file.')
    parser.add_argument('--model_path', dest='model_path',type=str,  help='Path to VGG_ILSVRC_16_layers pretrained model weight file i.e VGG_ILSVRC_16_layers.caffemodel')
    parser.add_argument('-i',dest='input_directory',help='Path to Directory containing images to be processed.')
    parser.add_argument('--filter',default = None ,dest='filter', help='Text file containing images names in the input directory to be processed. If no argument provided all images are processed.')
    parser.add_argument('--WITH_GPU', action='store_true', dest='WITH_GPU', help = 'Caffe uses GPU for feature extraction')
    parser.add_argument('-o',dest='out_directory',help='Output directory to store the generated features')
    
    args = parser.parse_args()
    
    input_directory = args.input_directory
    path_model_def_file = args.model_def_path
    path_model  = args.model_path
    filter_path = args.filter
    WITH_GPU    = args.WITH_GPU
    out_directory = args.out_directory

    # input_directory = '/Users/wulfe/Dropbox/Start/smile/data/frames/'
    # path_model_def_file = '/Users/wulfe/Dropbox/Start/smile/data/_temp/something.prototxt'
    # path_model  = '/Users/wulfe/Downloads/VGG_ILSVRC_16_layers.caffemodel'
    # filter_path = None
    # WITH_GPU = False
    # out_directory = '/Users/wulfe/Dropbox/Start/smile/data/features'

    input_directory = '/home/ubuntu/data/input/frames'
    path_model_def_file = '/home/ubuntu/models/proto/default.prototxt'
    path_model  = '/home/ubuntu/models/caffe/VGG_ILSVRC_16_layers.caffemodel'
    filter_path = None
    WITH_GPU = False
    out_directory = '/home/ubuntu/data/output'


    # check if files and directories exist - lets adopt this
    if not os.path.exists(out_directory):
        raise RuntimeError("Output directory does not exist %s"%(out_directory))
    
    if not os.path.exists(input_directory):
        raise RuntimeError("%s , Directory does not exist"%(input_directory))
    
    if not os.path.exists(path_model_def_file):
        raise RuntimeError("%s , Model definition file does not exist"%(path_model_def_file))
    
    if not os.path.exists(path_model):
        raise RuntimeError("%s , Path to pretrained model file does not exist"%(path_model))
    
    if not filter_path == None:
        imgs = open(filter_path,'r').read().splitlines()        
    else:
        imgs = os.listdir(input_directory)

    # this could be improved
    for f in imgs:
        if '.' in os.path.splitext(os.path.basename(f))[0]:
            imgs.remove(f)
    imgs = sort_img_paths(imgs)
    imgs = imgs[:]
    
    path_imgs = [os.path.join(input_directory, file) for file in imgs]
    print(path_imgs)
    # sort b/c this is a video and the order will matter
    # perhaps I should label each row with it's frame number and video?
    print('path_imgs[0]: {}'.format(path_imgs[0]))
    print('path_imgs[1]: {}'.format(path_imgs[1]))

    start_time = time.time()
    print "Feature Extraction for %d images starting now"%(len(path_imgs))
    feats = extract_visual_features(path_imgs, path_model_def_file, path_model, WITH_GPU)
    print "Total Duration for generating predictions %.2f seconds"%(time.time()-start_time)
    
    out_path = os.path.join(out_directory,'feats.csv')
    print "Saving prediction to disk %s"%(out_path)
    vgg_feats = {}
    vgg_feats['feats'] = feats.T
    print('type(vgg_feats): {}'.format(type(vgg_feats)))
    print('type(feats): {}'.format(type(feats)))
    print('feats.shape: {}'.format(feats.shape))
    print('feats[0]: {}'.format(feats[0]))
    print('feats[0][0]: {}'.format(feats[0][0]))
    # maybe adopt
    # guess not
    write_features_to_file(out_path, feats.T)

    #scipy.io.savemat(out_path , vgg_feats)
    
    print "Have a Good day!"
    
    
 